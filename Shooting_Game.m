...
function Shooting_Game()
   ...
   system_Name=text(0.5,2.3,'Shooting Game','Color','[0.3010 0.7450 0.9330]','EdgeColor','blue','FontWeight','bold','FontAngle','italic','Fontsize',30); 
   pause(1.5);
   system_announcement=text(0.35,1.7,'此射擊遊戲是用 左方向鍵、右方向鍵 控制綠色槍枝移動','Color','white','Fontsize',10);
   pause(1.5);
   system_announcement1= text(0.45,1.5,'並用 空白鍵 發射子彈  注意： 請將鍵盤輸入法換為','Color','white','Fontsize',10);
   pause(1.5);
   system_announcement2= text(1.21,1.2,'英文 輸入','Color','red','Fontsize',15);
   pause(8);
   delete(system_announcement);
   delete(system_announcement1);
   delete(system_announcement2);
   pause(1);
   delete(system_Name);
    ...
    while game_status==1 || game_status==2
        ...
        if player(1)>=30 && player(2)>=50 && boss_flag>=3
            ...
            while true
                ...
                if IsPress.e
                   systemEnd_announcement=text(1.0,1.5,'End the game','Color','white','Fontsize',15);
        ...
        game_num=game_num+1;
        system_announcement=text(0.3,1.5,['The ',num2str(game_num), ' th Play'],'Color','green','Fontsize',20);
        pause(1);
        delete(system_announcement);
